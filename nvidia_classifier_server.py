...
Wraps the NVIDIA prompt-task-and-complexity-classifier
Original version with fixed model loading.
"""
...
class CustomModel(nn.Module, PyTorchModelHubMixin):
    ...
    def process_logits(self, logits):
        ...
        for i, target in enumerate(["creativity_scope", "reasoning", "contextual_knowledge",
                                     "number_of_few_shots", "domain_knowledge",
                                     "no_label_reason", "constraint_ct"], start=1):
            result[target] = self.compute_results(logits[i], target=target)
        result["prompt_complexity_score"] = [
            round(0.35 * creativity + 0.25 * reasoning + 0.15 * constraint
    ...
    def forward(self, batch):
        ...
        logits = [self.heads[k](pooled) for k in range(len(list(self.target_sizes)))]
        return self.process_logits(logits)

MODEL_NAME = "nvidia/prompt-task-and-complexity-classifier"
print(f"[EcoRoute] Loading {MODEL_NAME} ...")

# FIXED: Manual config loading instead of AutoConfig
config_path = hf_hub_download(repo_id=MODEL_NAME, filename="config.json", force_download=False)
...
app = FastAPI(title="EcoRoute NVIDIA Classifier", version="1.0")
...
def count_constraints(text: str) -> int:
    lower = text.lower()
    hits = len(re.findall(r'\bwith\b', lower))
    hits += len(re.findall(r'\bthat (can|should|must|handles|supports)\b', lower))
    return hits
...
def classify(req: ClassifyRequest):
    ...
    creativity = float(result["creativity_scope"][0])
    reasoning = float(result["reasoning"][0])
    raw_scores = {
        "task_type_1": task_type, "task_type_2": result["task_type_2"][0],
        "task_type_prob": float(result["task_type_prob"][0]),
        "creativity_scope": creativity, "reasoning": reasoning,
        "contextual_knowledge": float(result["contextual_knowledge"][0]),
        "domain_knowledge": float(result["domain_knowledge"][0]),
        "constraint_ct": float(result["constraint_ct"][0]),
        "number_of_few_shots": float(result["number_of_few_shots"][0]),
    ...
    return ClassifyResponse(
        ...
        constraint_count=count_constraints(req.text),
